...
daily_sales$order_date <- as.Date(daily_sales$order_date, format = "%d/%m/%Y")
...
stacked_bar_data <- top_customers %>%
...
top_rating_products <- dbGetQuery(my_db, "
  SELECT p.id AS product_id, p.name AS product_name, p.brand, AVG(o.rating_review) AS avg_rating
  FROM products AS p
  JOIN orders AS o ON p.id = o.product_id
  WHERE o.rating_review IS NOT NULL
...
top_rating_products %>%
ggplot(aes(x = product_name, y = avg_rating, fill = brand)) +
  geom_bar(stat = "identity") +
  labs(title = "Top 10 Rated Products",
       x = "Product",
       y = "Average Rating",
       fill = "Brand") +
  ...
  geom_text(aes(label = round(avg_rating, 2)), vjust = -0.5, size = 3, color = "black") +
  coord_flip()
...
ggplot(aes(area = total_sales, fill = category_name, label = paste0(category_name, "\n", scales::dollar(total_sales)))) +
  geom_treemap() +
  geom_treemap_text(fontface = "bold", place = "centre", grow = TRUE, reflow = TRUE) +
...
# Convert order_date to Date format
xyz$order_date <- as.Date(xyz$order_date)

# Aggregate by month
xyz <- xyz %>%
  mutate(year_month = floor_date(order_date, "month")) %>%
  group_by(seller_id, year_month) %>%
  filter(seller_id %in% top_5_sellers$id)
